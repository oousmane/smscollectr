...
``` r
is_bad_sms(x, gauge = TRUE, sent_date = Sys.Date())
```
...
- sent_date:
  ...
  `Date(1)`. The date the message was received. The date rules (not in
  the future, not older than `SMSCOLLECTR_MAX_AGE` days) are evaluated
  relative to this date. Defaults to
