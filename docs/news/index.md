...
- **`fix_sms(gauge = FALSE)` implemented** — agro SMS values are now
  ...
  placeholders are preserved, decimal separators (`.`/`,`) are stripped
  so observer-written physical values conform to the x10 SMS convention
  (e.g. `43.0` → `430`), and trailing units/text are dropped from the
  numeric part. Vectorised over `x`, consistent with the gauge path.
...
- [`parse_sms()`](https://oousmane.github.io/smscollectr/reference/parse_sms.md)
  now also runs malformed agro SMS through `fix_sms(gauge = FALSE)`
  before parsing (previously only gauge SMS were fixed).
...
- `DESCRIPTION` description field trailing period added (R CMD check
  NOTE).
- [`read_sms()`](https://oousmane.github.io/smscollectr/reference/read_sms.md)
  sent-date parsing no longer fails when the system’s `LC_TIME` locale
  is not English. `raw$date` strings such as `"July 4, 2026 at 12:49AM"`
  were previously parsed with `as.Date(..., format = "%B %d, %Y")`,
  which depends on the OS locale to recognise English month names and
  silently returned `NA` on non-English systems. Parsing is now done
  with [`strptime()`](https://rdrr.io/r/base/strptime.html) inside
  `withr::with_locale(c(LC_TIME = "C"))`, so the month name is always
  read in the C locale regardless of the user’s OS settings.
- [`fix_sms()`](https://oousmane.github.io/smscollectr/reference/fix_sms.md)
  and `.parse_sms()` now check `length(d) == 0` before calling
  `is.na(d)` on the parsed date. A date string that fails to parse
  (e.g. an empty line, or the locale issue above) made
  [`as.Date()`](https://rdrr.io/r/base/as.Date.html)/[`strptime()`](https://rdrr.io/r/base/strptime.html)
  return a zero-length result, and
  [`is.na()`](https://rdrr.io/r/base/NA.html) on a zero-length vector
  raised `argument is of length zero`, aborting the whole parse instead
  of flagging that single message as unfixable.
- [`is_bad_sms()`](https://oousmane.github.io/smscollectr/reference/is_bad_sms.md)
  gains a `sent_date` argument (default
  [`Sys.Date()`](https://rdrr.io/r/base/Sys.time.html)) and no longer
  hardcodes the real system clock for its future/too-old checks.
  [`fix_sms()`](https://oousmane.github.io/smscollectr/reference/fix_sms.md)
  and `.parse_sms()` had already been made `sent_date`-aware, but
  [`is_bad_sms()`](https://oousmane.github.io/smscollectr/reference/is_bad_sms.md)
  was missed — so
  [`parse_sms()`](https://oousmane.github.io/smscollectr/reference/parse_sms.md)’s
  gauge-fixing decision and
  [`read_sms()`](https://oousmane.github.io/smscollectr/reference/read_sms.md)’s
  `struct_bad` flagging were still judging a message’s date against the
  moment the code happened to run, rather than the date it was actually
  sent, silently misclassifying otherwise-valid old messages as
...
- `.parse_sms()`, `.parse_val()`, `.parse_agro_sms()` marked `@noRd` —
  ...
  index.
- `withr` moved from `Suggests` to `Imports` — no longer just a
  test-mocking dependency now that
  [`read_sms()`](https://oousmane.github.io/smscollectr/reference/read_sms.md)
  uses
...
- 8 new tests covering `fix_sms(gauge = FALSE)` and `.fix_agro_value()`:
  value normalisation, date validation, malformed input, and
  vectorisation.
